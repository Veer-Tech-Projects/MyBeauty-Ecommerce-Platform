...
import "../../styles/AddressForm.css"; // custom styles (provided below)
...
export default function AddressForm({ onAddressSelected }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.fullname || !form.phone || !form.pincode || !form.house || !form.road || !form.city || !form.state) {
    ...
    try {
      if (editAddress) {
        await axios.put(`/api/addresses/${editAddress.id}`, form);
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/api/addresses/${toDeleteId}`);
      toast.success("Address removed");
      setShowDeleteModal(false);
      setToDeleteId(null);
      await fetchAddresses();
      // if deleted selected, clear selection
      if (selectedId === toDeleteId) setSelectedId(null);
    } catch (err) {
