...
import "../../styles/AdminAddProductPage.css";
...
/**
 * Notes:
 * - react-select used for category select with custom option & single-value renderer.
 * - isSearchable set to false to keep dropdown simple as requested.
 */
...
const AdminAddProductPage = () => {
    ...
    .then((res) => {
      ...
      const fixed = arr.map((c) => ({
        ...
        image: c.image.startsWith("http")
          ? c.image
          : `http://localhost:5000${c.image}`, // ✅ Fix: add Flask origin
      }));
      setCategories(fixed);
    })
    .catch((err) => {
  ...
  const [productData, setProductData] = useState({
    name: "",
    category: null, // react-select option object or null (optional)
    brand: "",
    description: "",
    images: [],
    enableSizeStock: false,
  ...
  const handleSubmit = async (e) => {
    ...
    const fieldsToAppend = {
      ...
      category_id: productData.category ? productData.category.value : "", // optional
    ...
    // variants payload and variant images
    if (productData.enableVariants) {
            ...
            <Card className="custom-card p-4 h-100">
              ...
              <Form.Group className="mb-3">
                ...
                <Select
                  ...
                  isClearable
                  isSearchable={true} // as requested: keep simple
              ...
              {/* toggles placed here as requested */}
