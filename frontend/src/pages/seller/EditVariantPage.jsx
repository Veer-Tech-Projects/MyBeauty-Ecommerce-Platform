...
export default function EditVariantPage() {
  ...
  useEffect(() => {
    const fetchVariant = async () => {
      try {
        const { data } = await axios.get(`/api/seller/variants/${id}`, {
        ...
        setFormData((prev) => ({
          ...
          images: [],
          imagePreviews: existingImages,  // e.g., ["/uploads/variant1.jpg", ...]
          removedImages: []
        }));
  ...
  const handleSubmit = (e) => {
    ...
    formData.images.forEach((file) => {
      if (file && typeof file !== "string") {
        payload.append("variant_images", file); // backend expects this
      }
    });
    ...
    axios
      .put(`/api/seller/variants/${id}`, payload, {
        headers: {
