...
export default function SellerProductsPage() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/seller/products/list-with-variants", {
  ...
  const handleToggleStatus = (productId, isVariant = false) => {
    const url = `http://localhost:5000/api/seller/products/${productId}/status${
  ...
  const confirmDelete = () => {
    ...
    const url = `http://localhost:5000/api/seller/products/${id}${isVariant ? "?is_variant=true" : ""}`;
                  ...
                  <td>
                    ...
                    {prod.discount > 0 && (
                      <span className="badge bg-danger-subtle text-danger ms-2">
                        {calculateDiscount(prod.price, prod.mrp)}% OFF
                      </span>
                    )}
                  </td>
                  <td>{prod.stock}</td>
                  ...
                  <td>
                    <div className="d-flex gap-2">
                      <button
                        ...
                        onClick={() => navigate(`/seller/edit-product/${prod.id}`)}
                      >
                            ...
                            <tbody>
                              {prod.variants.map((v, vidx) => (
                                <tr key={v.id} className="variant-row variant-indent small">
                                  ...
                                  <td className="text-muted">{prod.category_name}</td>
                                  <td className="text-muted">—</td>
                                  <td>
                                    <div className="d-flex gap-2">
                                      <button
                                        className="btn btn-sm btn-outline-secondary"
                                        onClick={() => navigate(`/seller/edit-variant/${v.id}`)}
                                      >
                                        <i className="bi bi-pencil"></i>
                                      </button>
                                      <button
